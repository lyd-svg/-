...
| 功能 | 实现 | 触发条件 |
|------|------|---------|
| 历史裁剪 | `_trim_history()` - 保留最近 N 轮对话 | 初始化时 `history_len` 参数（默认 8 轮） |
...
| `visualize_data` | 高级分组可视化（多系列） | PNG 路径 + Markdown 描述 |
| `draw_chart` | 基础统计图表 | PNG 路径 + Markdown 描述 |
| `get_chart_types` | 列出支持的图表类型 | Markdown 列表 |

**单引擎设计**：统一基于 matplotlib，pyecharts 代码保留但暂未接入。

| 引擎 | 工具 | 优势 | 适用场景 |
|------|------|------|---------|
| **matplotlib** | `visualize_data` | 支持多系列分组、堆叠、横向 | 含分组的复杂图表 |
| **matplotlib** | `draw_chart` | 基础柱/线/饼/散点，自动降级 | 单系列简单图表 |
...
visualize_data(data_json)
    ...
    ├── VIZ_AUTO_RULES           // 标题关键词匹配选型
    │   （趋势→line / 排行→hbar / 占比→stacked_bar）
    │
    └── mpl_plots (matplotlib)   // 统一走 matplotlib 输出 PNG

draw_chart(data_json)            // 基础图表
    │
    ├── auto_xy(data)
    ├── auto_chart_type(title)   // 标题关键词 + 数据特征选型
    │
    └── mpl_plots (matplotlib)
```
...
```
calculate("sqrt(25) * pi + 3**2")
    │
    ├── ast.parse(expression)           // 解析为 AST
    │
    ├── _SafeEvaluator.visit()          // AST 节点白名单校验
    │   ├── ✅ 允许：BinOp, UnaryOp, Num, List, Call(白名单函数)
    │   ├── ❌ 拒绝：`exec`/`eval`/`__import__`/`open` 等
    │   └── ❌ 拒绝：属性访问、赋值、import
    │
    └── 白名单函数列表
        ├── 基础数学：abs, round, int, float, min, max, sum
        ├── 幂指对：sqrt, pow, exp, log, log10, log2
        ├── 三角：sin, cos, tan, asin, acos, atan, degrees, radians
        ├── 取整：ceil, floor, trunc
        ├── 组合：factorial
        └── 常量：pi, e, inf, nan
```
...
```
RateLimiter(max_concurrent=10, max_per_second=50, max_queue=50)
（各服务可自定义：analysis/rag 服务使用 `(5, 20, 30)`，db 服务使用 `(10, 50, 50)`）
    │
...
> 注：Span 结束事件（`on_span_end`）当前未启用，因此不展示"工具调用结束"或"handoff 返回"的完成提示。
...
| 存储 | 内容 | 持久化路径 |
|------|------|-----------|
| ChromaDB 向量索引 | 文档分块的嵌入向量 | `vector_db/` |
...
history = InputItem[]  // SDK 的 InputItem 数组
    │
    ├── Trim 策略：保留最近 8 轮（可配置，UI 滑块 0~20）
...
| Volume | 挂载点 | 内容 |
|--------|--------|------|
| `vector_data` | `/app/vector_db/` | 向量索引 |
